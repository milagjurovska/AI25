...
class Balls(Problem):
    def __init__(self, initial, obstacles, goal, grid):
    ...
    def successor(self, state):
        ...
        for k in range(len(balls)):
            ...
            newx = x - 2
            newy = y + 2
            if self.valid((newx, newy), (x - 1, y + 1)):
                new_balls = [b for b in balls if b != (x - 1, y + 1) and b != (x, y)]
                new_balls.append((newx, newy))
                successors[fr"Gore Levo: (x={x},y={y})"] = tuple(new_balls)
            ...
            if self.valid((newx, newy), (x + 1, y + 1)):
                new_balls = [b for b in balls if b != (x + 1, y + 1) and b != (x, y)]
                new_balls.append((newx, newy))
                successors[fr"Gore Desno: (x={x},y={y})"] = tuple(new_balls)
            ...
            if self.valid((newx, newy), (x - 1, y - 1)):
                new_balls = [b for b in balls if b != (x - 1, y - 1) and b != (x, y)]
                new_balls.append((newx, newy))
                successors[fr"Dolu Levo: (x={x},y={y})"] = tuple(new_balls)
            ...
            if self.valid((newx, newy), (x + 1, y - 1)):
                new_balls = [b for b in balls if b != (x + 1, y - 1) and b != (x, y)]
                new_balls.append((newx, newy))
                successors[fr"Dolu Desno: (x={x},y={y})"] = tuple(new_balls)

            newx = x - 2
            if self.valid((newx, y), (x - 1, y)):
                new_balls = [b for b in balls if b != (x - 1, y) and b != (x, y)]
                new_balls.append((newx, y))
                successors[fr"Levo: (x={x},y={y})"] = tuple(new_balls)

            newx = x + 2
            if self.valid((newx, y), (x + 1, y)):
                new_balls = [b for b in balls if b != (x + 1, y) and b != (x, y)]
                new_balls.append((newx, y))
                successors[fr"Desno: (x={x},y={y})"] = tuple(new_balls)

        return successors

    def valid(self, topche, sledno):
        x, y = topche
        if not (0 <= x < self.grid and 0 <= y < self.grid):
            return False
        if (x, y) in self.obstacles:
            return False
        if sledno not in self.initial:
            return False
        return True
    ...
    def goal_test(self, state):
        return len(state) == 1 and self.goal == state[0]
...
if __name__ == "__main__":
    ...
    for i in range(M):
        line = input().split(",")
        topchinja.append((int(line[0]), int(line[1])))

    L = int(input())
    prepreki = []
    for j in range(L):
        line = input().split(",")
        prepreki.append((int(line[0]), int(line[1])))

    celta=(N//2, N-1)

    balls = Balls(tuple(topchinja), tuple(prepreki), celta, N)
