...
class GhostOnSkates(Problem):
    ...
    def successor(self, state):
        ...
        newx, newy = x, y+3
        if self.check_valid((newx, newy), self.walls, self.n):
            successors["Gore 3"] = (newx, newy)

        newx, newy = x, y + 2
        if self.check_valid((newx, newy), self.walls, self.n):
            successors["Gore 2"] = (newx, newy)

        newx, newy = x, y + 1
        if self.check_valid((newx, newy), self.walls, self.n):
            successors["Gore 1"] = (newx, newy)

        newx, newy = x+3, y
        if self.check_valid((newx,newy), self.walls, self.n):
            successors["Desno 3"] = (newx, newy)

        newx, newy = x + 2, y
        if self.check_valid((newx, newy), self.walls, self.n):
            successors["Desno 2"] = (newx, newy)

        newx, newy = x + 1, y
        if self.check_valid((newx, newy), self.walls, self.n):
            successors["Desno 1"] = (newx, newy)
    ...
    def h(self, node):
        state=node.state
        goal=self.goal
        return (abs(state[0]-goal[0])+abs(state[1]-goal[1]))/2
