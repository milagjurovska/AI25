...
if __name__ == '__main__':
    ...
    test_data=class_0[int(len(class_0)*x2):] + class_1[int(len(class_1)*x2):] + class_2[int(len(class_2)*x2):]

    train_nb=class_0[:int(len(class_0)*x1)] + class_1[:int(len(class_1)*x1)] + class_2[:int(len(class_2)*x1)]
    train_dt=class_0[int(len(class_0)*x1):int(len(class_0)*x2)] + class_1[int(len(class_1)*x1):int(len(class_1)*x2)] + class_2[int(len(class_2)*x1):int(len(class_2)*x2)]
    train_rf=class_0[:int(len(class_0)*x2)] + class_1[:int(len(class_1)*x2)] + class_2[:int(len(class_2)*x2)]
    ...
    X_train_nb=[row[:-1] for row in train_nb]
    y_train_nb=[row[-1] for row in train_nb]
    X_train_rf = [row[:-1] for row in train_rf]
    y_train_rf = [row[-1] for row in train_rf]
    X_train_dt = [row[:-1] for row in train_dt]
    y_train_dt = [row[-1] for row in train_dt]

    clf_nb.fit(X_train_nb, y_train_nb)
    clf_dt.fit(X_train_dt, y_train_dt)
    clf_rf.fit(X_train_rf, y_train_rf)

    ypred_nb=clf_nb.predict(X_test)
    ypred_dt=clf_dt.predict(X_test)
    ypred_rf=clf_rf.predict(X_test)

    accuracy=0

    for gt, pred1, pred2, pred3 in zip(y_test, ypred_nb, ypred_dt, ypred_rf):
        if gt == pred1 and gt == pred2 or gt == pred3 and gt == pred1 or gt == pred2 and gt == pred3:
            accuracy+=1

    print("Tochnost:", accuracy/len(y_test))
