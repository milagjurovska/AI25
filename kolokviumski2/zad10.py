...
if __name__ == '__main__':
    ...
    train_dataset=dataset[int(len(dataset)*x/100):]
    test_dataset=dataset[:int(len(dataset)*x/100)]
    ...
    dt_cls=DecisionTreeClassifier(criterion='gini', random_state=0)
    dt_cls.fit(train_X, train_y)
    features=list(dt_cls.feature_importances_)
    least_important=features.index(min(features))


    train_X_new = [row[:least_important] + row[least_important+1:] for row in train_X]
    test_X_new = [row[:least_important] + row[least_important+1:] for row in test_X]

    minmax=MinMaxScaler()
    stand=StandardScaler()
    minmax.fit(train_X_new)
    stand.fit(train_X_new)

    train_X_minmax=minmax.transform(train_X_new)
    train_X_stand=stand.transform(train_X_new)
    test_X_minmax=minmax.transform(test_X_new)
    test_X_stand=stand.transform(test_X_new)

    mlp_cls1=MLPClassifier(15, activation="relu", learning_rate_init=0.001, max_iter=200, random_state=0)
    mlp_cls1.fit(train_X_minmax, train_y)

    mlp_cls2=MLPClassifier(15, activation="relu", learning_rate_init=0.001, max_iter=200, random_state=0)
    mlp_cls2.fit(train_X_stand, train_y)

    print("Tocnost so StandardScaler:", mlp_cls2.score(test_X_stand, test_y))
    print("Tocnost so MinMaxScaler:", mlp_cls1.score(test_X_minmax, test_y))
