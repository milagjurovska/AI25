...
class Squares(Problem):
    ...
    def successor(self, state):
        ...
        for k in range(len(kvadrati)):
            if kvadrati[k]!= self.goal[k]:
                broj=str(k+1)

                newx, newy = kvadrati[k][0], kvadrati[k][1] + 1
                if self.check_valid([(newx, newy)] + kvadrati[:k] + kvadrati[k + 1:]):
                    new_state = list(kvadrati)
                    new_state[k] = (newx, newy)
                    succ["Pomesti kvadratche " + broj + " gore"] = tuple(new_state)

                newx, newy=kvadrati[k][0]-1, kvadrati[k][1]
                if self.check_valid([(newx, newy)] + kvadrati[:k] + kvadrati[k+1:]):
                    new_state = list(kvadrati)
                    new_state[k] = (newx, newy)
                    succ["Pomesti kvadratche "+broj+" levo"] = tuple(new_state)

                newx, newy = kvadrati[k][0], kvadrati[k][1] - 1
                if self.check_valid([(newx, newy)] + kvadrati[:k] + kvadrati[k+1:]):
                    new_state = list(kvadrati)
                    new_state[k] = (newx, newy)
                    succ["Pomesti kvadratche " + broj + " dolu"] = tuple(new_state)
                ...
                if self.check_valid([(newx, newy)] + kvadrati[:k] + kvadrati[k+1:]):
                    new_state = list(kvadrati)
                    new_state[k] = (newx, newy)
                    succ["Pomesti kvadratche " + broj + " desno"] = tuple(new_state)
