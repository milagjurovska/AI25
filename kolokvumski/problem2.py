...
class PillarProblem(Problem):
    def __init__(self, pochetok, goal):
        super().__init__(pochetok, goal)

    def successor(self, state):
        successors = dict()
        pillars = [list(p) for p in state]

        for pr in range(len(pillars)):
            if len(pillars[pr])==0:
                continue
            ...
            for p in range(len(pillars)):
                ...
                if len(pillars[p])==0 or pillars[p][-1] >= top:
                    new_pillars = [list(p) for p in pillars]
                    new_pillars[pr].pop()
                    new_pillars[p].append(top)

                    successors[f"MOVE TOP BLOCK FROM PILLAR {pr+1} TO PILLAR {p+1}"] = tuple([tuple(p) for p in new_pillars])
...
if __name__ == "__main__":
    initial = input().split(";")
    start = tuple(tuple(map(int, p.split(","))) if p else () for p in initial)

    kraj = input().split(";")
    end = tuple(tuple(map(int, p.split(","))) if p else () for p in kraj)

    pillar = PillarProblem(start, end)
