...
if __name__ == '__main__':
    ...
    train_data=class_0[:int(len(class_0)*x/100)] + class_1[:int(len(class_1)*x/100)]
    test_data=class_0[int(len(class_0)*x/100):] + class_1[int(len(class_1)*x/100):]
    ...
    clf_rf=RandomForestClassifier(criterion='entropy', n_estimators=4, random_state=0)
    clf_nn=MLPClassifier(10, activation="relu", learning_rate_init=0.001, random_state=0)
    ...
    accuracies={}
    accuracies["Naive Bayes"]=clf_nb.score(X_test, y_test)
    accuracies["Decision Tree"]=clf_dt.score(X_test, y_test)
    accuracies["Random Forest"]=clf_rf.score(X_test, y_test)
    accuracies["MLP"]=clf_nn.score(X_test, y_test)

    najgolema=max(accuracies, key=accuracies.get)

    print("Najgolema tocnost ima klasifikatorot", najgolema)

    clf=[]

    tp, fn = 0, 0
    for x_row, gt_class in zip(X_test, y_test):
        glas_0=0
        glas_1=0
        y_pred=clf_nb.predict([x_row])[0]
        if y_pred == 0:
            glas_0+=2 if najgolema == "Naive Bayes" else 1
        else:
            glas_1+=2 if najgolema == "Naive Bayes" else 1

        y_pred = clf_rf.predict([x_row])[0]
        if y_pred == 0:
            glas_0 += 2 if najgolema == "Random Forest" else 1
        else:
            glas_1 += 2 if najgolema == "Random Forest" else 1

        y_pred = clf_dt.predict([x_row])[0]
        if y_pred == 0:
            glas_0 += 2 if najgolema == "Decision Tree" else 1
        else:
            glas_1 += 2 if najgolema == "Decision Tree" else 1

        y_pred = clf_nn.predict([x_row])[0]
        if y_pred == 0:
            glas_0 += 2 if najgolema == "MLP" else 1
        else:
            glas_1 += 2 if najgolema == "MLP" else 1

        final = 1 if glas_1 < glas_0 else 0

        if final == 1 and final == gt_class:
            tp += 1
        elif final == 0 and final != gt_class:
            fn += 1

    print("Odzivot na kolekcijata so klasifikatori e", 1-tp/(tp+fn))
