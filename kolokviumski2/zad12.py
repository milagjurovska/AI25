...
if __name__ == '__main__':
    ...
    classifier=MLPClassifier(n,activation="relu",max_iter=20, learning_rate_init=learning_rate, random_state=0)

    train_dataset=dataset[:int(len(dataset)*0.8)]
    val_dataset=dataset[int(len(dataset)*0.8):]

    train_x=[row[:-1] for row in train_dataset]
    val_x=[row[:-1] for row in val_dataset]
    train_y=[row[-1] for row in train_dataset]
    val_y=[row[-1] for row in val_dataset]
    ...
    for sample_X, gt_class in zip(train_x, train_y):
        pred_class = classifier.predict([sample_X])[0]
        if gt_class == pred_class:
            accuracy_count += 1

    accuracy_tr = accuracy_count / len(train_dataset)

    accuracy_count=0

    for sample_X, gt_class in zip(val_x, val_y):
        pred_class = classifier.predict([sample_X])[0]
        if gt_class == pred_class:
            accuracy_count += 1

    accuracy_val = accuracy_count / len(val_dataset)

    if accuracy_val*1.15 < accuracy_tr:
        print("Se sluchuva overfitting")
        classifier1=MLPClassifier(n,activation="relu",max_iter=20, learning_rate_init=learning_rate, random_state=0)
        train_x=[row[:col] + row[col+1:] for row in train_x]
        test_x=[row[:col] + row[col+1:] for row in val_x]
        classifier1.fit(train_x,train_y)
        y_pred=classifier1.predict(line)[0]
        print(y_pred)
    else:
        print("Ne se sluchuva overfitting")
        y_pred = classifier.predict(line)[0]
        print(y_pred)
